...
def upload_documents_from_path(files_path: Path, meilisearch_config: dict):
    ...
    try:
        client = meilisearch.Client(
            ...
            meilisearch_config.get("api_key")
        )
        # Test connection
        client.health()
    ...
    try:
        ...
        from metadata_handler import BASE_FIELDS, METADATA_SCHEMA
    ...
    print(f"Total documents processed: {sum(r.get('count', 0) for r in responses if r.get('success'))}")
    print(f"Metadata errors: {len(metadata_errors)}")


def main():
    parser = argparse.ArgumentParser(description="Upload legislative debate data to Meilisearch")
